...
function deleteAccount() {
  request("php/deleteAccount.php", false, function (data) {
    ...
    switch (data) {
      ...
      case "1":
        document.getElementById("errs").innerHTML +=
          '<div class="err">Failed to delete account. Please try again later.</div>';
        break;
      case "2":
        document.getElementById("errs").innerHTML +=
      ...
      case "3":
        document.getElementById("errs").innerHTML +=
          '<div class="err">You are not logged in.</div>';
        break;
      case "4":
        document.getElementById("errs").innerHTML +=
    ...
    try {
      ...
      for (var i = 0; i < data.length; ++i) {
        switch (data[i]) {
          ...
          default:
            document.getElementById("errs").innerHTML +=
              '<div class="err">An unknown error occurred. Please try again later. (in the default)</div>';
        }
      }
    } catch (e) {
      document.getElementById("errs").innerHTML =
        '<div class="err">An unknown error occurred. Please try again later. (in the catch)</div>';
    }
    setTimeout(function () {
      document.getElementById("errs").style.transition = transition;
      document.getElementById("errs").style.opacity = 1;
...
// update.php
function update() {
  request("php/update.php", "#updateForm", function (data) {
    document.getElementById("errs").innerHTML = "";
    var transition = document.getElementById("errs").style.transition;
    document.getElementById("errs").style.transition = "none";
    document.getElementById("errs").style.opacity = 0;
...
function sendValidateEmailRequest() {
  request("php/sendValidationEmail.php", "#validateEmailForm", function (data) {
    ...
    switch (data) {
      case "0":
        document.getElementById("errs").innerHTML +=
          "<div>Email Sent... Check your email and click the link in the email to validate your email.</div>";
  ...
  request(
    ...
    function (data) {
      ...
      switch (data) {
        case "0":
          document.getElementById("errs").innerHTML +=
            "<div>An email has been sent if an account with that email exists</div>";
        ...
        case "3":
          document.getElementById("errs").innerHTML +=
            '<div class="err">You have exceeded your number of allowed reset requests per day. Try again later.</div>';
          break;
        case "4":
          document.getElementById("errs").innerHTML +=
            '<div class="err">Failed to connect to database. Please try again later.</div>';
    ...
    try {
      ...
      for (var i = 0; i < data.length; ++i) {
        switch (data[i]) {
          case 0:
            document.getElementById("errs").innerHTML +=
              '<div>Your password has been reset! You can now <a href="./login">login</a></div>';
          ...
          case 7:
            document.getElementById("errs").innerHTML +=
              '<div class="err">Invalid password reset request. If this is a mistake send a new request and click the link in the email.</div>';
            break;
          case 3:
            document.getElementById("errs").innerHTML +=
              '<div class="err">Password must contain: <ul><li>At least 8 characters</li><li>At least one lower case letter</li><li>At least one upper case letter</li><li>At least one number</li><li>At least one special character (~?!@#$%^&*)</li></ul></div>';
